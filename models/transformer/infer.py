...
class Inferer:
    ...
    def greedy_decode(self, model, source, source_mask, max_len):
    ...
    def infer(self, data, max_len, output_file=None):
        ...
        with torch.no_grad():
            ...
            dataset = BilingualDataset(dataAdapted, self.tokenizer_src, self.tokenizer_tgt, self.src_lang,
                                       self.tgt_lang,
                                       max_len)
            ...
            model_out = self.greedy_decode(self.model, encoder_input, encoder_mask, max_len)
    ...
    def load_model(self, path):
        self.model = get_model(self.config, self.tokenizer_src.get_vocab_size(),
                               self.tokenizer_tgt.get_vocab_size()).to(
            self.device)
